...
>
> **共通の日時方針**：`users`を含む全テーブルで`created_at`・`updated_at`を必須とする。`created_at`は行作成時、`updated_at`は行更新時のサーバー時刻を記録する。既存テーブルには更新時に`updated_at`を進める仕組みがなく、是正は未実装。追記専用イベントの日時列の扱いも実装時にこの共通方針と整合させる。`effective_at`はこれらと異なる、競合判定に使う補正済み操作時刻である。
>
