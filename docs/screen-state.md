...
stateDiagram-v2
    ...
    メニュー展開 --> 確認ダイアログ: 削除ボタン

    確認ダイアログ --> ラウンド一覧: キャンセル
    確認ダイアログ --> ラウンド一覧: 背景クリック
    確認ダイアログ --> 削除中: 確認ボタン

    削除中 --> ラウンド一覧: 削除失敗（メッセージ表示）
    削除中 --> ラウンド一覧: 削除成功（一覧から削除）

    classDef external stroke-dasharray: 4 4
    class RoundDetail,RoundNew external
```

| イベント | ラウンド一覧 |  | メニュー展開 |  | 確認ダイアログ |  | 削除中 |  |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| ラウンドカード | /rounds/[id] | 〇 | - |  | - |  | - |  |
...
| 背景クリック | - |  | - |  | ラウンド一覧 | 〇 | - |  |
| 確認ボタン | - |  | - |  | 削除中 | 〇 | - |  |
| 削除失敗 | - |  | - |  | - |  | ラウンド一覧（メッセージ表示） | 〇 |
| 削除成功 | - |  | - |  | - |  | ラウンド一覧から削除 | 〇 |

対応事項（実装済みだが修正が必要）:
- `deleteRound()`（Server Action内の削除処理）を、`/signin`の`signIn()`と同じ理由で通常のクライアント側`fetch()`に置き換える。ただしこの画面でそのレースコンディションが実際に確認されたわけではなく、`/signin`と同じ構造を持つことによる予防的な統一。対応要否は`/signin`での知見を踏まえて判断する
