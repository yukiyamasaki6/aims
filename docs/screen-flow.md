...
| `/signup` | サインアップ画面 | メール送信→認証コード入力→パスワード設定の3ステップ（同一URL内、実装済み） |
| `/reset-password` | パスワード再設定画面 | メール送信→認証コード入力→新パスワード設定の3ステップ（同一URL内。`/signup`と同じパターン） |
...
| `/rounds/new` | ラウンド作成 | 定型プリセットまたはカスタム（空で開始）を選ぶだけの入口。選択と同時に`rounds`を作成し`/rounds/[id]`へ遷移する。ラウンド名・実施日・種別・弓種・距離構成の入力は行わない（個人・チーム両コンテキストで共用） |
| `/rounds/[id]` | ラウンド入力・閲覧の統合画面 | コンテキストに依存しない共通の着地点。エンドごとに矢を記録し、合計・内訳を常時表示。独立したサマリー専用画面は設けない。ラウンド名・実施日・種別・弓種・距離構成（追加/削除・的の選択）の入力・編集もこの画面でのみ行う |
...
レフトパネルはURLを持たず、各画面上のオーバーレイとして開閉する状態であり、独立した画面ではない。
