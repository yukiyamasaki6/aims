...
│  │  ├─ new/
│  │  │  └─ page.tsx (Client)   ラウンド作成の入口（個人・チーム両コンテキストで共用）。選択と同時に`rounds`を作成し`[id]`へ遷移する
│  │  │     └─ RoundPresetSelect   定型プリセット選択（グローバル/自分の過去の構成）＋「カスタム（空で開始）」
│  │  └─ [id]/
│  │     └─ page.tsx (Client)   ラウンド入力・閲覧の統合画面（コンテキストに依存しない共通の着地点）
│  │        ├─ RoundConfigPanel    ラウンド名・実施日・種別・弓種・距離構成（追加/削除）・プリセット保存ボタンをまとめた折りたたみ式の編集区画（普段は要約1行、タップで展開）。距離ごとの総エンド数・エンドあたりの本数・的選択は各距離行がラウンド全体の折りたたみとは別に個別に折りたためる（要約1行↔展開）。総エンド数・エンドあたりの本数は`shots`が1件でも入力された距離では編集不可（削除して作り直す）
...
- ラウンドの属性（名前・実施日・種別・弓種）や距離構成（距離の追加・削除、距離ごとの的の選択）は`rounds/[id]`の`RoundConfigPanel`でのみ入力・編集し、`rounds/new`と同じ入力UIを重複して持たない。`rounds/new`は定型プリセットまたはカスタムを選ぶ入口に徹する。距離削除時に`shots`が存在する場合は確認を挟むが、削除自体は`shots.distance_id`の`on delete cascade`（スキーマ変更不要）で完結する。
