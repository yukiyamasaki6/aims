...
// 「正しく呼び出せているか（境界呼び出しの抑止・正常系の配線）」と「reducerの出力を正しく表示に反映できているか」を別個に検証する。
...
describe("SignInForm", () => {
  ...
  describe("境界呼び出しの抑止", () => {
    it("validateSignInFieldsがemailのエラーを返す場合、submit_invalidをdispatchしsignInWithPasswordを呼ばない", async () => {
      // Given
      const fieldErrors = { email: "メールアドレスの形式が正しくありません。" };
    ...
    it("validateSignInFieldsがpasswordのエラーを返す場合、submit_invalidをdispatchしsignInWithPasswordを呼ばない", async () => {
      // Given
      ...
      // Then
      expect(validateMock.fields).toHaveBeenCalledWith("user@example.com", "");
      await vi.waitFor(() => {
        expect(flow.reducer).toHaveBeenCalledWith(expect.anything(), {
          type: "submit_invalid",
          errors: fieldErrors,
    ...
    it("captcha未完了ではsubmit_invalidをdispatchしsignInWithPasswordを呼ばない", async () => {
      // Given
      const user = userEvent.setup();
      render(<SignInForm />);
  ...
  describe("正常系の配線", () => {
    it("送信はsubmit_startedをdispatchし正しい引数でsignInWithPasswordを呼び、成功時は/roundsへ遷移する", async () => {
      // Given
    ...
    it("成功時は送信中状態を解除せず、遷移完了前に再度押しても再送信しない", async () => {
      // Given
      auth.signInWithPassword.mockResolvedValue({ error: null });
      const user = userEvent.setup();
      render(<SignInForm />);
    ...
    it("送信中の二重クリックではsignInWithPasswordを1回しか呼ばない", async () => {
      // Given
  ...
  describe("送信中にアンマウントされた場合の副作用抑止", () => {
    it("エラー応答の前にアンマウントされた場合、captchaのリセットを行わない", async () => {
      // Given
      const deferred = Promise.withResolvers<{ error: AuthError | null }>();
      auth.signInWithPassword.mockReturnValue(deferred.promise);
      const user = userEvent.setup();
    ...
    it("例外発生の前にアンマウントされた場合、captchaのリセットを行わない", async () => {
      // Given
      const deferred = Promise.withResolvers<{ error: AuthError | null }>();
      auth.signInWithPassword.mockReturnValue(deferred.promise);
      const user = userEvent.setup();
    ...
    it("成功応答の前にアンマウントされた場合、/roundsへ遷移しない", async () => {
      // Given
      const deferred = Promise.withResolvers<{ error: AuthError | null }>();
      auth.signInWithPassword.mockReturnValue(deferred.promise);
      const user = userEvent.setup();
